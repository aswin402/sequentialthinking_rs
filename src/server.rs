...
impl SequentialThinkingServer {
    ...
    fn format_thought(&self, thought_data: &ThoughtData) -> String {
        ...
        let header_styled = format!("{} {}/{}{}", prefix, thought_number, total_thoughts, context.cyan());
        ...
        if let Some(conf) = thought_data.confidence_score {
            ...
            let plain_conf = format!("Confidence: {} ({:.0}%)", "★".repeat(stars_count) + &"☆".repeat(5 - stars_count), conf * 100.0);
    ...
    pub fn process_thought(&mut self, mut input: ThoughtData) -> Result<ToolResult, String> {
        ...
        if let (Some(_branch_from), Some(branch_id)) = (input.branch_from_thought, input.branch_id.as_ref()) {
            self.branches
